...
function match(r,s,currency='usd'){
 for(const [key,values] of Object.entries(s.dims)){if(s.ceco2507==='include'&&r.optionalCeco2507&&['req','profit','area'].includes(key))continue;if(values.size&&!values.has(clean(r[key])))return false;}
 if(s.review==='exclude'&&r.review||s.review==='only'&&!r.review)return false;
 if(s.ceco2507!=='include'&&r.optionalCeco2507)return false;
 ...
 if(s.search){const hay=[r.po,r.ot,r.doc,r.description,r.vendor,r.vendorId,r.otName,r.account,r.accountName,r.material,r.materialName,r.req,r.equipment,r.location,r.cecoContable,r.CECO_PO,r.costCenter,r.senderCostCenter,r.contract,r.sourceNote].join(' ').toLocaleLowerCase('es');if(!hay.includes(s.search.toLocaleLowerCase('es')))return false;}
 return true;
}
